...
export default function ModalCreateCategory({
  ...
  const onCreateCategory = async ({ name }: { name: string }) => {
    ...
    await fetchCreateCategory(token, {
      name: name,
    })
      .then((response) => {
        setIsloading(false);
        setSnackbar({
          isOpen: true,
          message: "Berhasil menambah kategori asset!",
          severity: "success",
        });
        onSuccess();
      })
      .catch((error) => {
        ...
        setSnackbar({
          isOpen: true,
          message: "Gagal menambah kategori asset!",
          severity: "error",
        });
      });
  };
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>Silahkan masukkan data kategori</DialogContentText>
          <TextField
            autoFocus
            required
            margin="dense"
